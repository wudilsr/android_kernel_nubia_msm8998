...
static void calculate_slop_values(void);

static ssize_t synaptics_rmi4_remap_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_remap_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(remap, (S_IRUGO | S_IWUSR),
			synaptics_rmi4_remap_show,
			synaptics_rmi4_remap_store),
...
static void calculate_slop_values()
{
	screen_slop = remap * 3;
	screen_left_edge = screen_slop;
	screen_right_edge = NUBIA_SCREEN_WIDTH - screen_slop;
	return;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	remap = input < 0 ? 0 : input;
    calculate_slop_values();
